...
Using both of these, we obtain the same output of $`r occ_count(taxonKey = sp_key)`$ *Calluna vulgaris* records mediated by GBIF. Note that this number will likely be higher for you as data is continually added to the GBIF indexing and this document here is frozen in time (it was last updated on `r Sys.Date()`).
...
Here is how *Caluna vulgaris* records are distributed across countries according to GBIF on `r Sys.Date()`:
<details>
...
occ_countries <- occ_count(taxonKey = sp_key,
                           facet = c("country"),
                           facetLimit = nrow(enumeration_country())
)
occ_countries <- cbind(occ_countries, 
                       data.frame(GBIF = "GBIF",
...
plot_df <- rbind(occ_countries[order(occ_countries$Records, decreasing = TRUE)[1:10], -1],
                 data.frame(
                   GBIF = "GBIF",
                   Countries = "Others",
...
As it turns out, roughly `r round(occ_NO$meta$count/occ_count(taxonKey = sp_key), 4)*100`% of *Calluna vulgaris* records mediated by GBIF fall within Norway.
...
```{r}
occ_year <- occ_search(taxonKey = sp_key,
                       country = "NO",
                       year = 2022)
occ_year$meta$count
```
...
```{r}
occ_window <- occ_search(taxonKey = sp_key,
                         country = "NO",
                         year = 2000:2022)
```
...
```{r}
sum(unlist(lapply(occ_window, FUN = function(x){x$meta$count})))
```
...
plot_df <- data.frame(Year = 2000:2022,
                      Records = unlist(lapply(occ_window, FUN = function(x){x$meta$count})),
                      Cumulative = cumsum(unlist(lapply(occ_window, FUN = function(x){x$meta$count})))
)
ggplot(data = plot_df, aes(x = as.factor(Year), y = Cumulative)) +
  ...
  theme_bw() +
  scale_fill_manual(name = 'Records',
...
```{r}
occ_human <- occ_search(taxonKey = sp_key,
                     country = "NO",
                     year = 2022,
...
These account for `r round(occ_human$meta$count/occ_year$meta$count, 4) * 100`% of all *Caluna vulgaris* observations in Norway throughout the year 2022. So, what might be the basis of record for the remaining `r occ_year$meta$count-occ_human$meta$count` records?

### Preserved Specimen

As it turns out, the remaining `r occ_year$meta$count-occ_human$meta$count` records are based on preserved specimen:
```{r}
occ_preserved <- occ_search(taxonKey = sp_key,
                     country = "NO",
                     year = 2022,
...
basis_occ <- data.frame(basisofRecord = c("FOSSIL_SPECIMEN", "HUMAN_OBSERVATION", "MATERIAL_CITATION", "MATERIAL_SAMPLE",
...
basis_occ$Records <- pbapply(basis_occ, MARGIN = 1, FUN = function(x){
  occ_search(taxonKey = sp_key,
             country = x[2],
             basisOfRecord = x[1])$meta$count
})
...
links2$value <- unlist(apply(links2, MARGIN = 1, FUN = function(x){
  # print(x)
  if(x[1] == "Others"){
    aggregate(data = basis_occ[!(basis_occ$Country %in% nodes$name) & basis_occ$Var1 %in% as.character(x[2]), ],
              x = Records ~ 1, FUN = sum)
  }else{
...
nodes_plot <- rbind(nodes,
                    data.frame(name = unique(basis_occ$Var1),
                               group = "c")
)
...
SN <- sankeyNetwork(Links = links_plot, Nodes = nodes_plot,
              Source = "IDsource", Target = "IDtarget",
              Value = "value", NodeID = "name",
              colourScale = my_color, NodeGroup = "group",
...
```{r}
occ_present <- occ_search(taxonKey = sp_key,
                          country = "NO",
                          year = 2022,
                          basisOfRecord = "HUMAN_OBSERVATION",
...
```{r}
occ_absent <- occ_search(taxonKey = sp_key,
                         country = "NO",
                         year = 2022,
                         basisOfRecord = "HUMAN_OBSERVATION",
...
## query basis of record per country
basis_occ <- expand.grid(c("PRESENT", "ABSENT"),
                         c("FOSSIL_SPECIMEN", "HUMAN_OBSERVATION", "MATERIAL_CITATION", "MATERIAL_SAMPLE",
                           "LIVING_SPECIMEN", "MACHINE_OBSERVATION", "OBSERVATION", "PRESERVED_SPECIMEN", "OCCURRENCE"))

basis_occ$Records <- pbapply(basis_occ, MARGIN = 1, FUN = function(x){
  occ_search(taxonKey = sp_key,
             basisOfRecord = x[2],
             occurrenceStatus = x[1])$meta$count
})
...
nodes_final <- rbind(nodes_plot,
                    data.frame(name = unique(basis_occ$target),
                               group = "d")
)
...
SN <- sankeyNetwork(Links = links_final, Nodes = nodes_final,
                    Source = "IDsource", Target = "IDtarget",
                    Value = "value", NodeID = "name",
                    colourScale = my_color, NodeGroup = "group",
