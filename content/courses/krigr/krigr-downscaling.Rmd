...
```{r CovShape, results = 'hide', fig.keep = 'all', fig.height = 4, eval= FALSE}
...
Notice that despite the covariate rasters (and input rasters, for that matter) containing `r ncell(Covs_ls[[1]])` and `r as.numeric(ncell(Covs_ls[[2]]))` for training and target resolution respectively, we only obtain data for `r sum(!is.na(values(Covs_ls[[1]])))` and `r sum(!is.na(values(Covs_ls[[2]])))` cells respectively due to our specification of `SpatialPolygons`. This will come in handy when doing the statistical interpolation (see [this section](/courses/krigr/kriging/#spatial-limitation) for details).
...
```{r KrigShp, fig.keep = 'all', fig.height = 16, message = TRUE}
...
```{r, KrigPlot, fig.height = 20, results = "asis"}
...
As you can see, the elevation patterns show up clearly in our kriged air temperature output. Furthermore, you can see that our certainty of Kriging predictions drops on the 04/01/1995 in comparison to the preceding days. However, do keep in mind that a maximum standard error of `r round(maxValue(SpatialPolygonsKrig[[2]]), 3)` (for each layer of our output respectively) on a total range of data of `r round(maxValue(SpatialPolygonsKrig[[1]])-minValue(SpatialPolygonsKrig[[1]]), 3)` (again, for each layer in the output respectively) is evident of a downscaling result we can be confident in. We also demonstrated reliability of kriging in [this publication (Figure 3)](https://iopscience.iop.org/article/10.1088/1748-9326/ac39bf).  

Finally, this `SpatialPolygons`-informed downscaling took roughly `r as.numeric(round(ShpTime))` minutes on my machine (this may vary drastically on other devices).
...
```{r, KrigRExt, echo = FALSE, results = "hide", fig.height = 20}
ExtRaw <- stack(file.path(Dir.Data, "ExtentRaw.nc"))
Covs_ls <- download_DEM(Train_ras = ExtRaw,
                        Target_res = .02,
                        Dir = Dir.Covariates,
                        Keep_Temporary = TRUE)
ExtKrig <- krigR(Data = ExtRaw,
...
```{r, KrigRPts, results = "hide", fig.height = 16}
...
1. `extent` specification (`r sum(!is.na(values(ExtRaw)))` data cells in training resolution) - roughly 30 minutes  
2. `SpatialPolygons` specification (`r sum(!is.na(values(SpatialPolygonsRaw)))` data cells in training resolution) - roughly 4 minutes  
3. Point (`data.frame`) specification (`r sum(!is.na(values(PtsRaw)))` data cells in training resolution) - roughly 30 seconds
...
```{r CovShapeloc, results = 'hide', fig.keep = 'all', fig.height = 4}
Covs_ls <- download_DEM(Train_ras = SpatialPolygonsRaw,
                        Target_res = .02,
                        Shape = Shape_shp,
                        Dir = Dir.Covariates,
...
```{r KrigShpLocal, fig.keep = 'all', results = "hide", fig.height = 20}
...
```{r KrigShpLocalDiff, fig.keep = 'all', results = "hide", fig.height = 10}
...
- `SpatialPoylgonsEns` - [created here](/courses/krigr/download/#dynamical-data-uncertainty-1); [download here](https://github.com/ErikKusch/Homepage/raw/master/content/courses/krigr/Data/SpatialPolygonsEns.nc) containing dynamical uncertainty  

First, we load the data and assign them to objects with shorter names:
```{r, fig.keep = 'all', results = "hide", fig.height = 10}
SpatialPolygonsEns <- stack(file.path(Dir.Data, "SpatialPolygonsEns.nc"))
DynUnc <- SpatialPolygonsEns
KrigUnc <- SpatialPolygonsKrig[[2]]
```

Next, we need to align the rasters of statistical uncertainty (resolution: `r round(res(KrigUnc)[1], 3)`) and dynamical uncertainty (resolution: `r round(res(DynUnc)[1], 3)`). As you can see, these are of differing resolutions and so cannot easily be combined using raster math. Instead, we first disaggregate the coarser-resolution raster (`DynUnc`) as disaggregation does not attempt any interpolation thus preserving the data, but representing it with smaller cells. To fix final remaining alignment issues, we allow for some resampling between the two raster:
```{r, fig.keep = 'all', results = "hide", fig.height = 10}
